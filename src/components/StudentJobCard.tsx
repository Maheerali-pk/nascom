...
import { IJobPosting } from "../apis/student/getJobList";
...
const StudentJobCard: React.FC<IJobPosting> = (props) => {
   const location =
      props.location.type === "OFFICE"
         ? props.location.city + ", " + props.location.state
         : locationToStringObject[props.location.type];
   const timeAvail = timeAvailToStringObject["FR"];
   const stipend = useMemo(() => {
      if (props.compensation.stipendType === "FIXED") {
         return `${props.compensation.stipend} /${"month"}`;
      } else {
         return `${props.compensation.st} - ${props.maxStipend} /${
            props?.stipendPeriod?.toLowerCase?.() || "month"
         }`;
      }
